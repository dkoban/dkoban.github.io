...
1. Randomly sample 15,000 tweet IDs from GWU's [Tweet Sets Library]( https://tweetsets.library.gwu.edu/datasets) in the following collections:
...
3. Randomly sample 12,000 unique user IDs from the 15,000 tweets (some users are duplicated in the initial 15K sample).
...
6. Compare distribution of CAP scores across data sets.
7. Compare proportion of bot category scores > k across data sets (e.g., how many accounts had astroturf scores greater than k in each data set? did one data set have siginificantly more than others?)
8. Calculate correlation between Botometer and BotometerLite scores.

## Results

The following preliminary results explore the similarity between Botometer and BotometerLite scores for 10,000 users from the Coronavirus and random sample of tweets. 
...
#random <- read_csv("~/Documents/EM6574/covid_bot_scores_1Nov2020.csv") 
...
cat_tbl <- data_frame(
  ...
  "Coronavirus" = c(nrow(corona),
                    format_counts(corona_bot_prop$astro, nrow(corona)),
  ...
  "Random" = c(nrow(random),
               ...
               format_counts(random_bot_prop$fake, nrow(random)),
               format_counts(random_bot_prop$spammer, nrow(random)),
               format_counts(random_bot_prop$finance, nrow(random)),
               format_counts(random_bot_prop$self_declared, nrow(random)),
...
High Astroturf, Fake Follower, and Financial scores are uncommon.
...
all_data <- bind_rows(corona, fiveg)
